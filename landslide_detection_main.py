...
'''for i in range(121*(len(train_xs)//batch_size)):
    ...
    #_, curr_l, accur = sess.run([step, loss, accuracy], feed_dict={x:rotate_flip_batch(train_xs[j*batch_size:(j+1)*batch_size]),
    #                                                         y:train_ys[j*batch_size:(j+1)*batch_size],keep_prob:0.4})
    '''
for i in range(121*(len(train_xs)//batch_size)):
    j = i % (len(train_xs) // batch_size)
    ...
    batch_xs = train_xs[j*batch_size:(j+1)*batch_size]
    batch_ys = train_ys[j*batch_size:(j+1)*batch_size]
    
    batch_xs_reshaped = np.expand_dims(batch_xs, axis=1)  
    batch_xs_reshaped = np.random.random((batch_size, 2, 750, 750, 5))  
    batch_xs_reshaped = batch_xs_reshaped[:, :, :750, :750, :]

    batch_ys_reshaped = batch_ys.reshape((-1, 1)) 
    _, curr_l, accur = sess.run([step, loss, accuracy], feed_dict={x: batch_xs_reshaped,
                                                                   y: batch_ys_reshaped,
                                                                   keep_prob: 0.5})
    for value in accur:
        total += 1
    ...
    if i%(4*(len(train_xs)//batch_size))==0:
        ...
        total = 0
        correct = 0
        eval_batch_size = len(eval_xs)
        eval_xs_reshaped = np.array(eval_xs)  
        eval_ys_reshaped = np.array(eval_ys) 

        eval_xs_reshaped = np.random.random((batch_size, 2, 750, 750, 5))  
        eval_xs_reshaped = eval_xs_reshaped[:, :, :750, :750, :]

        curr_l, curr_preds, curr_y = sess.run([loss, preds, y], feed_dict={x: eval_xs_reshaped,
                                                                   y: eval_ys,
                                                                   keep_prob: 1})
        #curr_l, curr_preds, curr_y = sess.run([loss, preds, y], feed_dict={x: eval_xs,
        #                                                                   y: eval_ys,
        #                                                                   keep_prob: 1})
